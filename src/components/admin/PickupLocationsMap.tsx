...
type Pin = { id: string; label: string; lat: number; lng: number; isDriver: boolean }
    ...
    const init = async () => {
      ...
      const makeIcon = (color: string) => L.divIcon({
        html: `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="30" viewBox="0 0 24 32">
          <path d="M12 0C7.6 0 4 3.6 4 8c0 5.4 8 24 8 24s8-18.6 8-24c0-4.4-3.6-8-8-8z" fill="${color}"/>
      ...
      const passengerIcon = makeIcon('#059669')
      const driverIcon = makeIcon('#0284c7')

      for (const pin of pins) {
        const marker = L.marker([pin.lat, pin.lng], { icon: pin.isDriver ? driverIcon : passengerIcon }).addTo(map)
        const popupId = `pickup-address-${pin.id}`
        const mapsUrl = `https://www.google.com/maps?q=${pin.lat},${pin.lng}`
        marker.bindPopup(
          `<div style="min-width:180px">
            <div style="font-weight:600;margin-bottom:4px">${escapeHtml(pin.label)}${pin.isDriver ? ` <span style="color:#0284c7;font-weight:500">(${escapeHtml(dict.driverLabel)})</span>` : ''}</div>
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-4 mt-2 text-xs text-stone-500">
        <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-full bg-sky-600 inline-block" /> {dict.legendDriver}</span>
        <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-full bg-emerald-600 inline-block" /> {dict.legendPassenger}</span>
      </div>
    </div>
  )
}
