...
/**
 * Given either a host's or a friend's ViaFerrataParticipant id, resolves the full pair.
 * `friendId` is null when the participant did not bring a friend.
 */
export async function resolveViaFerrataPair(
  tx: Tx,
  participantId: string,
): Promise<{ hostId: string; friendId: string | null }> {
  const participant = await tx.viaFerrataParticipant.findUnique({
