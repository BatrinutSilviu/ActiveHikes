...
function ResetPasswordForm() {
  ...
  const d = useDict().auth.resetPassword
    ...
    <div className="min-h-screen bg-stone-950 flex items-center justify-center px-4 py-12">
      ...
      <div className="w-full max-w-sm relative">
        ...
        <div className="bg-white rounded-3xl shadow-xl shadow-stone-200/60 border border-stone-100 p-8">
          {!token ? (
            <p className="text-stone-600 text-sm text-center">{d.missingToken}</p>
          ) : (
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="block text-xs font-semibold text-stone-500 uppercase tracking-wide mb-1.5">{d.newPassword}</label>
              ...
              <div>
                <label className="block text-xs font-semibold text-stone-500 uppercase tracking-wide mb-1.5">{d.confirmPassword}</label>
